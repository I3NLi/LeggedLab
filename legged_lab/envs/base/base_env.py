...
class BaseEnv(VecEnv):
    ...
    def init_buffers(self):
        ...
        self._yaw_only_command_prob = float(
            np.clip(getattr(self.cfg.commands, "yaw_only_command_prob", 0.0), 0.0, 1.0 - self._straight_command_prob)
        )
        self._straight_command_env_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
        self._yaw_only_command_env_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
        ...
        self._static_log_info = {
            ...
            "Config/yaw_only_command_prob": float(self._yaw_only_command_prob),
        }
    ...
    def reset(self, env_ids):
        ...
        self._resample_command_mode_masks(env_ids)
        self._apply_command_mode_masks()
    ...
    def _compute_command_generator(self):
        ...
        if len(resample_env_ids) > 0:
            ...
            if self.command_metrics_enabled:
                self._resample_command_mode_masks(resample_env_ids)
        self.command_generator._update_command()
        if self.command_metrics_enabled:
            self._apply_command_mode_masks()
        self._update_command_slew()

    def _resample_command_mode_masks(self, env_ids):
        if len(env_ids) == 0:
            return
        if self._straight_command_prob <= 0.0 and self._yaw_only_command_prob <= 0.0:
            self._straight_command_env_buf[env_ids] = False
            self._yaw_only_command_env_buf[env_ids] = False
            return
        command_mode_sample = torch.rand(len(env_ids), device=self.device)
        straight_mask = command_mode_sample < self._straight_command_prob
        yaw_only_mask = (command_mode_sample >= self._straight_command_prob) & (
            command_mode_sample < self._straight_command_prob + self._yaw_only_command_prob
        )
        self._straight_command_env_buf[env_ids] = straight_mask
        self._yaw_only_command_env_buf[env_ids] = yaw_only_mask

    def _apply_command_mode_masks(self):
        if self._straight_command_prob <= 0.0 and self._yaw_only_command_prob <= 0.0:
            return
        ...
        yaw_only_env_ids = self._yaw_only_command_env_buf.nonzero(as_tuple=False).flatten()
        if len(yaw_only_env_ids) > 0:
            self.command_generator.vel_command_b[yaw_only_env_ids, 1] = 0.0
