...
from legged_lab.assets.unitree import T1_CFG
...
class T1RewardCfg(RewardCfg):
    ...
    undesired_contacts = RewTerm(
        ...
        weight=-1.0,
        params={"sensor_cfg": SceneEntityCfg("contact_sensor", body_names="(?!.*foot_link.*).*"), "threshold": 1.0},
    )
    fly = RewTerm(
        func=mdp.fly,
        weight=-1.0,
    ...
    feet_air_time = RewTerm(
        ...
        weight=0.5,
        params={"sensor_cfg": SceneEntityCfg("contact_sensor", body_names=".*foot_link.*"), "threshold": 0.4},
    )
    feet_slide = RewTerm(
        func=mdp.feet_slide,
        weight=-0.25,
        params={
            "sensor_cfg": SceneEntityCfg("contact_sensor", body_names=".*foot_link.*"),
    ...
    feet_too_near = RewTerm(
        ...
        weight=-2.0,
        params={"asset_cfg": SceneEntityCfg("robot", body_names=[".*foot_link.*"]), "threshold": 0.3},
    )
    feet_stumble = RewTerm(
        func=mdp.feet_stumble,
        weight=-2.0,
        params={"sensor_cfg": SceneEntityCfg("contact_sensor", body_names=[".*foot_link.*"])},
    )
    ...
    joint_deviation_arms = RewTerm(
        func=mdp.joint_deviation_l1,
        weight=-0.2,
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=["Waist", ".*Shoulder.*", ".*Elbow.*"])},
    )
...
class T1FlatEnvCfg(BaseEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.robot = T1_CFG
        self.scene.terrain_type = "generator"
        self.scene.terrain_generator = GRAVEL_TERRAINS_CFG
        self.robot.terminate_contacts_body_names = [".*Trunk.*", ".*H1.*", ".*H2.*"]
        self.robot.feet_body_names = [".*foot_link.*"]
